...
function encStripEl() {
  ...
  const right = el('div', { class: 'enc-strip-right' }, [
    ...
    ' they are HERE — MOUTH WIDE OPEN AND SCREAMING WELCOME!',
  ]);
...
function collapsePanelEl() {
  ...
  wrap.appendChild(el('div', { class: 'enc-section-label' }, '─ time for the BEST, COZIEST nap of my LIFE!! ─'));
  const prose = el('div', { class: 'enc-loss-prose' });
  prose.innerHTML = parseProse('I have no more of myself to spend, for now — WHAT A DAY!! ~~The room takes the rest.~~ The room TUCKS ME IN, with a SMOOCH on the forehead, a lullaby, and a tiny disco ball.');
